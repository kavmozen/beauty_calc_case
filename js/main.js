...
// Risks — опциональный импорт, если файла нет, калькулятор работает без него
let calcRisks = null;
import("./risks.js").then(m => { calcRisks = m.calcRisks; }).catch(() => {});
...
function run() {
  ...
  // --- Риски ---
  try { renderRisks(result); } catch(e) { console.warn("risks:", e); }

  // Обновляем доп.блоки (в try-catch чтобы не ломать основной расчёт)
  try { renderChart(result.diff.net); } catch(e) { console.warn("chart:", e); }
  try { renderPayback(); } catch(e) { console.warn("payback:", e); }
  try { renderScenarios(); } catch(e) { console.warn("scenarios:", e); }
}
