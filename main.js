...
// =========================================================
function renderPayback(monthlyDiff) {
  const costEl = getEl("implCost");
  const monthsEl = getEl("paybackMonths");
  const profitEl = getEl("profitYear");
  if (!costEl || !monthsEl || !profitEl) return;

  const cost = Math.max(0, num(costEl.value, 0));

  if (monthlyDiff <= 0) {
    monthsEl.textContent = "Не окупается";
    profitEl.textContent = fmtRub(monthlyDiff * 12 - cost);
    return;
  }

  const months = Math.ceil(cost / monthlyDiff);
  monthsEl.textContent = months <= 12 ? months + " мес." : "> 12 мес.";

  const yearProfit = monthlyDiff * 12 - cost;
  profitEl.textContent = fmtSignedRub(yearProfit);
}

const implCostEl = getEl("implCost");
if (implCostEl) {
  implCostEl.addEventListener("input", () => {
    if (lastResult) renderPayback(lastResult.diff.net);
  });
}
...
function exportPdf() {
  ...
  // Temporarily hide inputs for cleaner PDF
  const wasPresentation = document.body.classList.contains("presentation-mode");
  if (!wasPresentation) document.body.classList.add("presentation-mode");
  ...
  html2pdf()
    ...
    .save()
    .then(() => {
      if (!wasPresentation) document.body.classList.remove("presentation-mode");
    });
}
...
function runWithExtras() {
  ...
  renderPayback(lastResult.diff.net);
  renderScenarios();
}
